"""`calibrate-intrinsics` CLI: camera-intrinsics calibration from video,
...
from chuck_dreamer.perception.config import (
...
def _parse_resolution(raw: str) -> tuple[int, int]:
  ...
  if len(parts) != 2 or not all(p.strip().isdigit() for p in parts):
    raise click.UsageError(
      f"--resolution must be WxH (e.g. 1920x1080), got {raw!r}.")
...
@click.option("--episodes", "episode_specs", type=str, multiple=True,
              metavar="SPEC",
              help="EpisodeSpec, e.g. user/cal#1:0-200:5 (repeatable).")
@click.option("--camera", "camera", is_flag=False, flag_value="__default__",
              default=None, metavar="INDEX",
              help="Live preview + capture; optional device index "
                   "(default: real.camera.source). Incompatible with "
                   "--video/--episodes.")
@click.option("--resolution", default=None, metavar="WxH",
...
def calibrate_intrinsics_cmd(ctx, camera_id: str | None,
  ...
  if using_camera and using_files:
    raise click.UsageError("--camera is incompatible with --video / --episodes.")
  if resolution is not None and not using_camera:
    raise click.UsageError("--resolution only applies to --camera.")
  ...
  if not using_camera and not using_files:
    raise click.UsageError(
      "provide at least one source: --video, --episodes, or --camera.")
  ...
  if using_camera:
    ...
    assert camera is not None  # guarded by using_camera above
    device_arg: str = camera
    ...
    ck_size = (int(ol_cfg.checkerboard_size[0]), int(ol_cfg.checkerboard_size[1]))
    frame_iters.append(_frames_from_camera(device, real_cam, ck_size, stride=stride))
  else:
    for vid in videos:
      sources.append(f"video:{vid}")
      frame_iters.append(_frames_from_video(vid, stride=stride))
