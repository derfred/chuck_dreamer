...
if TYPE_CHECKING:
  from chuck_dreamer.perception.estimator import ObjectPose
...
def prompt_episodes_cmd(ctx, sources: tuple[str, ...],
  ...
  if ranges:
    ...
    click.echo("\nAll keyframes gathered. Next step: run `import-lerobot run` for "
...
def _run_augment_pass(pending: list[tuple[str, int, int]],
  ...
  """
  from lerobot.datasets.lerobot_dataset import LeRobotDataset  # type: ignore
  from chuck_dreamer.perception.estimator import ObjectPoseEstimator
...
def _anchor_pose(ds, ep_fr: int, ep_to: int, anchor_rel: int, prompt,
  ...
  from chuck_dreamer.perception.estimator import _EpisodeContext
...
def _pose_silhouette_area(pose, estimator) -> int:
  ...
  object.
  """
  from chuck_dreamer.perception.render import project_silhouette, transform_object
...
def _render_review_canvas(p: _Proposal,
  ...
  import cv2
  from chuck_dreamer.perception.render import project_visible_edges, transform_object
