...
A :class:`Run` builds the stage list per episode from the importer's
flags: it resolves the enabled stage names into a dependency-ordered list
(auto-pulling each stage's ``requires``). The importer iterates it calling
``stage.apply``; ``import-lerobot --doctor`` iterates the stage list *per
episode* and prints each stage's :meth:`Stage.requirements`, so neither
the loop nor the doctor hard-codes any single stage's details and
per-episode requirements are surfaced.
