...
def test_assemble_recording_playlist_renames_and_builds_coherent_vod(tmp_path):
  # An explicit recording is assembled from raw-*.ts copied out of the ring (in
  # write order; the suffix may be non-contiguous if a ring segment was skipped).
  # Assembly must rename to contiguous seg-*.ts and write a VOD index.m3u8 whose
  # entries all exist on disk (the integration test's playback invariant).
  d = tmp_path / "recordings" / "explicit" / "rec-1"
  d.mkdir(parents=True)
  (d / "raw-00000.ts").write_bytes(b"a")
  (d / "raw-00002.ts").write_bytes(b"bb")  # gap: a ring segment was skipped

  n = assemble_recording_playlist(d, segment_seconds=2)
  assert n == 2

  names = sorted(p.name for p in d.iterdir())
  # raw-*.ts renamed contiguously; only clean HLS files remain.
...
def test_assemble_recording_playlist_empty_dir_is_safe(tmp_path):
  # A recording window that captured no ring segments: assembly must not crash
  # and yields an empty-but-valid playlist.
