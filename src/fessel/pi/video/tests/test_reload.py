...
def test_apply_config_updates_recording_values():
  # One `recording` section describes both the ring encode and recordings (the
  # ring IS the recording buffer). New values apply on the next capture spawn.
  app = _app()
  app.apply_config(
    {
      "recording": {
  ...
  assert app._rec_cfg["resolution"] == "1920x1080"
  assert app._rec_cfg["fps"] == 25
  assert app._rec_cfg["bitrate_bps"] == 9_000_000
  assert app._rec_cfg["max_lookback_seconds"] == 300.0
  # Defaults still fill keys the reload didn't set.
  assert app._rec_cfg["segment_seconds"] == 2
  # The SM's start-timeout is applied live.
...
def test_apply_config_recording_takes_effect_on_next_spawn():
  ...
  app = _app()
  assert app._rec_cfg["bitrate_bps"] == 2_000_000  # default
...
def test_validate_config_rejects_bad_values():
  # One `recording` section (the ring IS the recording buffer): resolution is the
  # camera operating point the ring encodes at.
  with pytest.raises(ValueError):
    VideoApp.validate_config({"recording": {"resolution": "not-a-res"}})
  with pytest.raises(ValueError):
    VideoApp.validate_config({"recording": {"fps": 0}})
  with pytest.raises(ValueError):
    VideoApp.validate_config({"recording": {"bitrate_bps": -1}})
  with pytest.raises(ValueError):
    VideoApp.validate_config({"recording": {"segment_seconds": 0}})
  with pytest.raises(ValueError):
    VideoApp.validate_config({"recording": {"start_timeout_s": 0}})
  with pytest.raises(ValueError):
    VideoApp.validate_config({"recording": {"max_lookback_seconds": 0}})
